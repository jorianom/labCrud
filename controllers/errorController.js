...
const handleCastErrorDB = err => {
  const message = `Campo inválido en ${err.path}: ${err.value}.`;
...
module.exports = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV.trim() === 'development') {
    ...
    || error._message === "Department validation failed"
    || error._message === "Municipality validation failed"
    || error._message === "Place validation failed")
